...
public class TouchManager : MonoBehaviour
{
    ...
    [Header("ConfiguracionMarcadores")]
    [Tooltip("Prefab que se instanciara en la posicion del toque")]
    [SerializeField] private GameObject prefabMarcadorToque;
    [Tooltip("Distancia default desde la camara para instanciar marcadores")]
    ...
    private void Awake()
    {
        if (Instancia == null)
        {
            ...
            if (camaraPrincipal == null)
                Debug.LogError($"{gameObject.name}: No se encontro la camara principal con tag 'MainCamera'");
    ...
    private void ManejarToqueIniciado(Finger dedo)
    {
        ...
        if (depuracion)
            Debug.LogWarning($"{gameObject.name}: Toque iniciado con dedo indice {dedo.index}");
    ...
    private void InstanciarMarcador(Vector3 posicion)
    {
        ...
        {
            if (depuracion)
                Debug.LogWarning($"{gameObject.name}: Prefab marcador no asignado en TouchManager");
        }
    }
