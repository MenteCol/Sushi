...
public class CortePlatos : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
    ...
    [Header("Referencia a la línea UI para mostrar dirección")]
    ...
    private void Start()
    {
        ...
        {
            if (mostrarLogs) Debug.Log($"[PDollar] Loaded gesture XML: {gestureXml.name}");
            Gesture gesture = GestureIO.ReadGestureFromXML(gestureXml.text);
            trainingSet.Add(gesture);
            if (mostrarLogs) Debug.Log($"[PDollar] Parsed gesture template: {gesture.Name}");
        }
        ...
        foreach (var filePath in Directory.GetFiles(Application.persistentDataPath, "*.xml"))
        {
            ...
            trainingSet.Add(gesture);
            if (mostrarLogs) Debug.Log($"[PDollar] Loaded user gesture from: {Path.GetFileName(filePath)} (Name: {gesture.Name})");
        }

        if (mostrarLogs) Debug.Log($"[PDollar] Total gestures loaded: {trainingSet.Count}");

        foreach (var g in trainingSet)
        {
            if (mostrarLogs) Debug.Log($"[PDollar] Gesture in training set: {g.Name}");
        }
    ...
    private void EndDrawing()
    {
        ...
        if (Detectar && points.Count >= minPuntosParaDetectar && controladorInstancias.areaActivada)
        {
            ...
            if (longitudTramo >= minLineLength && controladorInstancias.areaActivada)
            {
                ...
                if (mostrarLogs) Debug.Log("Gesto: " + gestureResult.GestureClass + " | Puntuación: " + gestureResult.Score);
    ...
    {
        MostrarLineaUI(); // Mostrar la línea solo al hacer el corte
        ...
        Invoke(nameof(OcultarLineaUI), 0.5f); // Ocultar la línea tras 0.5 segundos (ajusta el tiempo si quieres)
    }

    // --- NUEVO: Orientación, posición y longitud de la línea UI ---
    private void MostrarLineaUI()
    {
        if (lineaUI == null || points.Count < 2) return;

        // 1. Calcular ángulo y longitud
        ...
        // 2. Convertir a espacio UI (Canvas Overlay)
        Vector2 uiPos;
        RectTransform parentRect = lineaUI.parent as RectTransform;
        RectTransformUtility.ScreenPointToLocalPointInRectangle(
            parentRect,
            primerPunto,
            null, // null para Canvas Overlay
            out uiPos
        );
        ...
        // 4. Escalar la línea (asumiendo que su eje X es la longitud)
